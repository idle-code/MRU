  ...
  switch(m_action) {
    case artist:
      if(file.tag()->artist() == TagLib::String::null)
        throw MetatagBase::Exception(glue_cast<UnicodeString>("No artist tag defined in file"));
      return glue_cast<UnicodeString>(file.tag()->artist());
    case title:
      if(file.tag()->title() == TagLib::String::null)
        throw MetatagBase::Exception(glue_cast<UnicodeString>("No title tag defined in file"));
      return glue_cast<UnicodeString>(file.tag()->title());
    case album:
      if(file.tag()->album() == TagLib::String::null)
        throw MetatagBase::Exception(glue_cast<UnicodeString>("No album tag defined in file"));
      return glue_cast<UnicodeString>(file.tag()->album());
    case year:
      if(file.tag()->year() == 0)
        throw MetatagBase::Exception(glue_cast<UnicodeString>("No year tag defined in file"));
      return glue_cast<UnicodeString>((int)file.tag()->year());
    case comment:
      if(file.tag()->comment() == TagLib::String::null)
        throw MetatagBase::Exception(glue_cast<UnicodeString>("No comment tag defined in file"));
      return glue_cast<UnicodeString>(file.tag()->comment());
    default:
      throw MetatagBase::Exception(glue_cast<UnicodeString>("Unsupported action: ") + glue_cast<UnicodeString>((int)m_action));
  }
...
METATAG_EXPORTS_START
  EXPORT_METATAG_PLUGIN_FACTORY(mru::AudioMetatag, "Audio")
METATAG_EXPORTS_END
